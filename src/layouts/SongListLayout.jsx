    ...
    return (
        <div 
            ...
            <div className="hidden md:flex flex-col gap-4 current-info flex-1/3 xl:flex-1/4 p-4 ps-0 h-full">
                <div className="text-shadow-[rgba(0,0,0,0.5)\ 0px\ 4px\ 6px] text-shadow-2xl side-title text-2xl">
                    Now Playing
                </div>
                <div className="side-controls-container flex flex-col gap-4 flex-grow">
                    ...
                    {/* <div className="shadow-[rgba(0,0,0,0.5)] shadow-xl overflow-hidden rounded-2xl">
                        <CommentBox comment={currentSong.comment} className={'flex-1'} />
                    </div> */}
...
function ControlsBox ({ song, className }) {
    return ( song &&
        <div className={"controls-container rounded-2xl overflow-hidden " + className}>
            <Image image={song.image_uri} className="w-full h-full">
                <div className="controls-content backdrop-blur-2xl p-2 h-full">
                    {/* <img src={song.image_uri} /> */}
                    <Image image={song.image_uri} className="rounded-2xl overflow-hidden h-full">
                        <div className="info dark-alpha-5 p-4 flex flex-col h-full justify-end">
                            <ArtistImage artistName={song.artist} className="flex-grow rounded-2xl" />
                            <div className="artist text-lg font-semibold mt-4">{ song.artist }</div>
                            <div className="album font-semibold">{ song.album }</div>
                            <div className="song font-semibold">{ song.song }</div>
